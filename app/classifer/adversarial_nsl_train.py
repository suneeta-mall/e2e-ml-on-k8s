...
from pylib import dataset_for_split
from pylib import download_input

from train import load_image_train, load_image_test
...
if __name__ == "__main__":
    ...
    validation_dataset = validation_slice.map(load_image_test,
                                              num_parallel_calls=TUNE_CONF). \
        map(convert_to_dictionaries). \
        batch(FLAGS.batch_size)

    test_dataset = test_slice.map(load_image_test,
                                  num_parallel_calls=TUNE_CONF). \
        map(convert_to_dictionaries). \
        batch(FLAGS.batch_size)
    ...
    history = adv_model.fit(training_dataset, epochs=FLAGS.epochs,
                            steps_per_epoch=FLAGS.steps_per_epoch,
                            validation_data=validation_dataset,
                            callbacks=[cp_callback, tensorboard_callback, lr_ctrl_callback, es_callback])
    adv_model.save(os.path.join(FLAGS.output, 'adv_classifier_model.h5'))
