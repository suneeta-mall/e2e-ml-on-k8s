...
if [ ! -z "${KUBEFLOW_TAG}" ]; then
  KUBEFLOW_VERSION=${KUBEFLOW_TAG:1}
else
  echo -e "${YELLOW}WARNING: KUBEFLOW_VERSION or KUBEFLOW_TAG is not defined. Defaulting to master.${NC}"
...
infer_minikube_settings() {
  ...
  if [[ $PLATFORM == $LINUX ]]; then
    ...
    dd=$(df --b G . | awk '{print $4}' | tail -1 | sed 's/[A-Za-z]//g')
  fi
  if [[ $dd -lt 40 ]]; then
